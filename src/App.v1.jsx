...
export default function BoardGame() {
  ...
  // Cowrie shell visual state
  const [cowrieShells, setCowrieShells] = useState([0, 0, 0, 0]); // 0=tails (flat), 1=heads (curved up)
  ...
  const rollDice = () => {
    ...
    // Animate the shells tumbling (random states rapidly)
    let animationCount = 0;
    const animationInterval = setInterval(() => {
      setCowrieShells(Array(4).fill(0).map(() => Math.random() > 0.5 ? 1 : 0));
      animationCount++;
      if (animationCount >= 8) {
      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-gradient-to-br from-white via-gray-50 to-gray-100 rounded-xl shadow-2xl p-6 w-72 border border-gray-200">
            ...
            {/* Visual Cowrie Shells */}
            <div className="flex justify-center gap-2 mb-3">
              {cowrieShells.map((shell, i) => (
                <div
                  key={i}
                  className={`relative transition-all duration-150 ${isRolling ? 'animate-bounce' : ''}`}
                  style={{ animationDelay: `${i * 50}ms` }}
                >
                  {/* Cowrie shell shape */}
                  <div
                    className={`w-8 h-12 rounded-full relative overflow-hidden transition-all duration-200 ${
                      shell === 1
                        ? 'bg-gradient-to-b from-amber-100 via-amber-50 to-white shadow-[0_4px_8px_rgba(0,0,0,0.3),inset_0_-8px_12px_rgba(139,69,19,0.3)]' // Heads - curved up (mouth visible)
                        : 'bg-gradient-to-b from-amber-200 via-amber-300 to-amber-400 shadow-[0_2px_4px_rgba(0,0,0,0.2),inset_0_2px_4px_rgba(255,255,255,0.3)]' // Tails - flat back
                    }`}
                  >
                    {/* Shell texture lines */}
                    {shell === 1 ? (
                      // Mouth side - show teeth/ridges pattern
                      <div className="absolute inset-0 flex flex-col justify-center items-center">
                        <div className="w-1 h-6 bg-gradient-to-b from-amber-300 via-amber-400 to-amber-300 rounded-full opacity-80"></div>
                      </div>
                    ) : (
                      // Back side - smooth with subtle lines
                      <div className="absolute inset-0">
                        <div className="absolute top-2 left-1/2 -translate-x-1/2 w-4 h-1 bg-amber-500/30 rounded-full"></div>
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 w-5 h-0.5 bg-amber-500/20 rounded-full"></div>
                        <div className="absolute top-6 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-amber-500/20 rounded-full"></div>
                      </div>
                    )}
                  </div>
                  {/* Shell label */}
                  <div className={`absolute -bottom-4 left-1/2 -translate-x-1/2 text-[10px] font-bold ${shell === 1 ? 'text-green-600' : 'text-gray-400'}`}>
                    {shell === 1 ? 'H' : 'T'}
                  </div>
                </div>
              ))}
            </div>
            {/* Heads count indicator */}
            <div className="text-center text-xs text-gray-500 mb-2">
              {cowrieShells.reduce((a, b) => a + b, 0)} heads = <span className="font-bold text-amber-600">
                {{ 4: 8, 3: 3, 2: 2, 1: 1, 0: 4 }[cowrieShells.reduce((a, b) => a + b, 0)]}
              </span>
            </div>

            <button
              onClick={rollDice}
              disabled={!canRoll || winner || computerPlayers[colors[currentPlayer]] || (onlineMode && colors[currentPlayer] !== myColor) || isRolling}
              className={`w-full px-4 py-3 rounded-lg font-bold text-white text-lg mb-4 transition-all duration-300 ease-out ${
                !canRoll || winner || computerPlayers[colors[currentPlayer]] || (onlineMode && colors[currentPlayer] !== myColor) || isRolling
                  ? 'bg-gray-400 cursor-not-allowed'
                  : 'bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 hover:shadow-lg active:scale-95'
              }`}
            >
              {isRolling ? 'ROLLING...' : onlineMode && colors[currentPlayer] !== myColor ? 'WAITING...' : canRoll ? 'ROLL COWRIES' : 'USE YOUR ROLLS'}
            </button>
